...
heartNUM2=select(heart,chol:disease)  # those from chol till disease selected
...
heartNEW7=mutate(heart, cholLevel=if_else(chol>250,"highrisk",(if_else(chol>200, "normal",(if_else(chol>175,"healthy","poor"))))))
...
count(heart, chest_pain, disease)
...
heart$cats <- case_when(
  heart$age >  30 & heart$age <= 40  ~ "Middle",
  heart$age >  40 & heart$age <=50   ~ "older",
  heart$age >  50 & heart$age <=65   ~ "senior", 
  heart$age >  65 & heart$age <= 90  ~ "Super senior",
  heart$age >  90 & heart$age <= 125 ~ "Super Hero"
)

View(heart)

heart=select(heart, -11)
...
groupHeartDisease3=group_by(heart, disease, chest_pain)            
            
  summarize(groupHeartDisease3,
          count=n(),
          avgAge=mean(age, na.rm=TRUE),
          sdAge=sd(age, na.rm=TRUE),
          medAge=median(age, na.rm=TRUE),
...
# challenge > group by sex,disease > analyse min,max,mean,count
#                                   cholesterol
...
dwishop=src_mysql(dbname="chinook", 
                  user='root', 
                  password='nu1977man', 
                  host='localhost')
